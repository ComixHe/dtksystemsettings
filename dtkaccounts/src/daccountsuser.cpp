...
#include "../include/daccountsuser.h"
...
DAccountsUser::DAccountsUser(const QString &path, const QString &systemPath, QObject *parent)
    : QObject(parent)
    , d_ptr(new DAccountsUserPrivate(path, systemPath, this))
{
}
...
QDateTime DAccountsUser::createdTime() const
{
    ...
    if (bash.exists() and bash.isFile())
        time = Dutils::getCreatedTimeFromFile(bash.absoluteFilePath());
    if (time == 0) {
        QFileInfo custom(UserConfigDir + userName());
        if (custom.exists() and custom.isFile())
            time = Dutils::getCreatedTimeFromFile(custom.absoluteFilePath());
    }
    return QDateTime::fromSecsSinceEpoch(time);
}
